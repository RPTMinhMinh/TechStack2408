...
function getAverageGrade(findByID: number): void {
    let sum: number = 0;
    let total: number = 0
...
// neu khum -> error
function findStudentByName(findByName: string): void {
    let check: boolean = false
    let student: Student | null = null;
    for (let element of students) {
...
//push
function addStudent(add: Student): void {
    students.push(add);
...
//neu khong -> error
function removeStudentById(deleteByID: number): void {
    let check = false
