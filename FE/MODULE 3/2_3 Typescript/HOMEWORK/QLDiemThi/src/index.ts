...
function calculateAverageScoreBySubject(subjectName: string):void{
    let sum: number = 0;
    let total: number = 0;
    // let score: ScoreData | null = null
    const generalSubject: ScoreData[] = []
...
function getTopScorerBySubject(subjectName: string): void{
    let generalSubject: ScoreData[] = []
    for(let element of scores){
        if(element.subject.toLowerCase().trim().includes(subjectName.toLowerCase().trim())){
            generalSubject.push(element)
        }
...
function getStudentScores(id: number):void {
    // let check: boolean = false;
    let student: ScoreData[] = []
    for(let element of scores){
